...
**`docker-compose.staging.yml`** - a middle ground, currently incomplete in two ways. First, a known security gap: postgres and redis ports are bound to `0.0.0.0` instead of `127.0.0.1`, which makes them reachable from outside the host. Second, and more urgent: every service in this file references `env_file: staging.env`, but no `staging.env` exists anywhere in `infra/` - only `.env.example`, `local.env`, and `prod.env` do. As written, this compose file cannot currently start at all. It also has no `tusd` service, so resumable uploads aren't available there even once the missing env file is added. Both issues should be fixed before any real staging deployment.
...
MinIO uses three buckets (created at startup or manually via the console; names below are the shipped defaults from `infra/local.env`/`infra/.env.example` - the env var names are uppercase, the bucket names themselves are lowercase-with-hyphens):
- **`vod-videos`** (`minio_bucket_videos`) - the original uploaded video file. Also where tusd writes resumable uploads, under a `tus-uploads/` object prefix, since both upload paths share this same bucket
- **`vod-thumbnails`** (`minio_bucket_thumbnails`) - cover images, stored similarly
- **`vod-processed`** (`minio_bucket_processed_videos`) - HLS segments and manifests, organized as `{video_id}/segments/master.m3u8` and `{video_id}/segments/{quality}/playlist.m3u8`
...
```
celery -A app.celery_app:celery_app worker --loglevel=INFO --concurrency=N
```

Concurrency is tuned per environment, not a single fixed value: **3** in local dev (`docker-compose.local.yml`), **1** in production (`docker-compose.yml`), **2** in the staging file. Each video processing run is CPU-intensive (FFmpeg), so more concurrency without more CPU just causes slowdowns - tune this to match your hardware if you change it.

### tusd

Runs as its own container, image `tusproject/tusd:v2.9.2` (pinned, not floating). It's the resumable-upload ingest server - see [15_RESUMABLE_UPLOADS.md](./15_RESUMABLE_UPLOADS.md) for the full picture. It writes uploaded parts directly to MinIO via S3 multipart upload (`-s3-bucket=${minio_bucket_videos}`) and calls FastAPI's hook endpoint (`-hooks-http=http://api:8000/internal/tus/hooks?secret=...`) before an upload starts and after it finishes. It has no `ports:` entry in local dev - Caddy is the only way to reach it, at `/files/*`. In production it additionally publishes `127.0.0.1:1080:1080`, needed because Caddy runs with `network_mode: host` there and can't resolve `tusd` by Docker service-name DNS the way the other services can.
...
Three routes exist alongside the catch-all: `/files/*` fronts tusd for the resumable-upload data plane, `/internal/tus/hooks/uploads/*` is the one `/internal` route meant to be reachable from outside (the admin-authenticated status bridge the frontend polls), and a blanket 404 on the rest of `/internal/*` keeps the hook POST route's shared secret (passed as a query param) out of the public internet and out of Caddy's own access log. tusd itself calls the hook POST route directly over the internal Docker network, never through this Caddy config.
...
```
The `JWT_SECRET_KEY` must be a strong random string in production. Generate one: `openssl rand -hex 32`. (The `Settings` class's own Python-level default for `access_token_expire_minutes` is 30, but every shipped env file - `local.env` and `.env.example` alike - overrides it to 60, so 60 is what you'll actually get.)
...
TUS_PART_SIZE_MB=50
TUS_MAX_CONCURRENT_UPLOADS=8
TUS_MAX_FILE_SIZE_GB=50
TUS_ALLOWED_MIME_TYPES=video/mp4,video/quicktime,video/webm
TUS_HOOK_SHARED_SECRET=generate-a-real-secret
TUS_ADMISSION_TTL_HOURS=2
```
See [15_RESUMABLE_UPLOADS.md](./15_RESUMABLE_UPLOADS.md) for what each of these controls.
...
**Staging can't currently start** - `docker-compose.staging.yml` references `env_file: staging.env` on every service, but that file doesn't exist anywhere in `infra/`. Until it's created, `docker compose -f docker-compose.staging.yml up` fails immediately, which also means the port-binding issue below can't currently be exercised in practice.

**Staging exposes database ports** - once the missing env file above is addressed, note that postgres and redis are bound to `0.0.0.0` in that same compose file. These should be `127.0.0.1` (or removed entirely, since the services don't need external access).
