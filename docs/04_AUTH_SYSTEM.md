...
**Access token** - short-lived (60 minutes), stateless. Attached as `Authorization: Bearer <token>` to every API request. The server validates it by checking the cryptographic signature - no database lookup required. This is fast. The payload contains: `user_id`, `email`, and `role`.
...
The tradeoff: stateless access tokens can't be revoked before they expire. A user who is banned still has a valid access token for up to 60 minutes. This is an acceptable tradeoff at this scale, and the 60-minute expiry limits the damage window.
...
6. Creates an **access token** (JWT, 60 min expiry) with payload `{user_id, email, role}`
...
Access tokens expire in 60 minutes. Rather than log the user out, the frontend automatically refreshes them.
...
This is a single-session logout. The access token remains valid for up to 60 more minutes, but since it's short-lived, this is acceptable. A "logout from all devices" feature would set `is_revoked = True` on ALL refresh tokens for the user.
...
`get_current_user`, defined in `core/dependencies.py`, reads the `Authorization: Bearer <token>` header, decodes the JWT, validates it, and fetches the user from the database. Every `/videos/*` route uses this version.
